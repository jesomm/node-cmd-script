...
module.exports = function getAllMatchingFilesUnderPath(currentPath, options) {
    ...
    if (files) {
        ...
        output = addToOutput(output, fileStrings, options, currentPath);
    ...
    if (dirs && dirs.length > 0) {
        dirs.forEach(currentDir => {
            if (shouldNotIgnoreDir(currentDir, options)) {
                ...
                var nextPath = `${ currentPath }\\${ currentDir }`;
                var subDirFile = getAllMatchingFilesUnderPath(nextPath, options);
                output = addToOutput(output, subDirFile, options, nextPath);
            }
        });
    }
